...
// Messages prefix
new const g_szPrefix[ ] = "[Dr.FioriGinal.Ro]";
...
new g_iMsgHideWeapon, g_iMsgCrosshair, g_iMsgMoney;
new g_iMaxplayers, g_iHudSync, g_iHudSync2, g_iLastTerr, g_iThinker;
new g_iSemiClip[ 33 ], g_Lifes[ 33 ];

new bool:ChoosedPistol[MAX_PLAYERS+1], Pistol[MAX_PLAYERS+1];
...
public plugin_init( ) {
	...
	if( get_pcvar_num( g_pToggle ) ) {
		...
		//register_clcmd( "say !choosepistol", "ChoosePistol");
...
public EventRandromize( ) {
	...
	if( iTeam == CS_TEAM_T || iTeam == CS_TEAM_CT ) {
		...
		set_user_gravity(iRandomPlayer, 0.0);
		
		new szName[ 32 ];
		get_user_name( iRandomPlayer, szName, 31 );
		
		for( i = 0; i < iNum; i++ )
			client_print_color(iPlayers[ i ], print_team_red, "^3%s^4 %L", g_szPrefix, iPlayers[ i ], "DR_NOW_TERR", szName);
...
public EventRoundStart( ) {
	...
	if( g_bFirstRound ) {
		...
		for( i = 0; i < iNum; i++ ) {
			...
			client_print_color( iPlayers[ i ], print_team_red, "^3%s^1 %L", g_szPrefix, iPlayers[ i ], "DR_STARTING_CC" );
		}
...
public CheckTerrorists( ) {
	...
	if( iTerrors == 0 ) {
		for( i = 0; i < iNum; i++ ) {
			client_print_color(iPlayers[ i ], print_team_red, "^3%s^1 %L", g_szPrefix, iPlayers[ i ], "DR_NO_DETECT");
...
public EventDeath( ) {
	...
	if( cs_get_user_team( iVictim ) == CS_TEAM_CT && g_Lifes[ iVictim ] > 0 ) {
		...
		if( iCTcount > 1 ) {
			...
			client_print_color( iVictim, print_team_red, "^3%s^1 %L", g_szPrefix, iVictim, "DR_LIFE_RESPAWN" );
		}
		/*else
			client_print_color( iVictim, print_team_red, "^3%s^1 %L", g_szPrefix, iVictim, "DR_LIFE_CANT" );*/
	}
...
public CmdShowlifes( id ) {
	if( get_pcvar_num( g_pLifeSystem ) == 0 ) {
		client_print_color( id, print_team_red, "^3%s^1 %L", g_szPrefix, id, "DR_LIFE_DISABLE" );
		return PLUGIN_HANDLED;
	}
	
	if( g_Lifes[ id ] > 0 )
		client_print_color( id, print_team_red, "^3%s^1 %L", g_szPrefix, id, "DR_LIFE_CC_COUNT", g_Lifes[ id ] );
	else
		client_print_color( id, print_team_red, "^3%s^1 %L", g_szPrefix, id, "DR_LIFE_CC_NO" );
...
public FwdFullPack( es, e, ent, host, flags, player, pSet ) {
	...
	/*static classname[15]; // timer
	pev(ent, pev_classname, classname, charsmax(classname));
	if (equal(classname, "start_portal") || equal(classname, "finish_portal"))
		return FMRES_IGNORED;	*/
...
public FwdStartFrame( ) {
	...
	static iPlayers[32], iPlayers2[32], iNum, iNum2, iPlayer, iPlayer2, i, j;
	get_players( iPlayers, iNum, "ache", "CT" );
	get_players( iPlayers2, iNum2, "ache", "SPECTATOR" );
	
	for (i = 0; i < iNum2; ++i)
	{
		iPlayers[iNum++] = iPlayers2[i];
	}
...
public FwdHamPlayerSpawn( id ) {
	...
	if( g_iFakeplayer == id ) {
		...
		/*if( get_pcvar_num( g_pGiveUsp ) && !g_bHauntedHouse )
			set_task( 10.0, "GiveUsp", id );*/
...
StripWeaponsFromSlot(id, const Slot)
{
        if(Slot < 0 || Slot > 5)
        {
                return;
        ...
        new CopyEntIndex;
        new WeaponEnt = get_pdata_cbase(id, m_rgpPlayerItems_CBasePlayer[Slot], XoPlayer);
        while(pev_valid(WeaponEnt))
        {
                CopyEntIndex = WeaponEnt;
                WeaponEnt = get_pdata_cbase(WeaponEnt, m_pNext, XoWeapon);
                if(ExecuteHamB(Ham_RemovePlayerItem, id, CopyEntIndex))
                {
                        ExecuteHamB(Ham_Item_Kill, CopyEntIndex);
                        set_pev(id, pev_weapons, pev(id, pev_weapons) & ~(1<< get_pdata_int(CopyEntIndex, m_iId, XoWeapon)));
                }
        }
}
/*
public ChoosePistol(id)
{
	if ( !(get_user_flags(id) & ADMIN_LEVEL_H) )
	{
		client_print_color(id, print_team_blue, "^4[^3Dr.FioriGinal.Ro^4] Nu ai Revive, nu poți folosi această comandă.");
		return PLUGIN_HANDLED;
	}
	
	if ( fm_get_user_team( id ) != 2 )
	{
		client_print_color(id, print_team_blue, "^4[^3Dr.FioriGinal.Ro^4] Nu eşti counter, nu poți folosi această comandă.");
		return PLUGIN_HANDLED;
	}
	
	if ( !is_user_alive(id) )
	{
		client_print_color(id, print_team_blue, "^4[^3Dr.FioriGinal.Ro^4] Nu eşti in viață, nu poți folosi această comandă.");
		return PLUGIN_HANDLED;
	}
	
	new menu = menu_create("Alege-ti un pistol :", "ChoosedAnswer");
	menu_additem(menu, "USP", "", 0);
	menu_additem(menu, "Glock 18", "", 0);
	menu_additem(menu, "p228", "", 0);
	menu_additem(menu, "Dual Beretas", "", 0);
	menu_additem(menu, "FiveSeven", "", 0);
	menu_setprop(menu, MPROP_EXIT, MEXIT_ALL);
	menu_display(id, menu, 0);
	return PLUGIN_HANDLED;
}*/

public ChoosedAnswer(id, menu, item)
{
	StripWeaponsFromSlot(id, 2);
...
public GiveUsp( const id ) {
	...
	if( is_user_alive( id ) ) 
	{
		...
		{
			new pistol = random(5);
			if ( get_user_flags( id ) & ADMIN_LEVEL_H && ChoosedPistol[id])
			{
				pistol = Pistol[id];
			}
			switch( pistol )
