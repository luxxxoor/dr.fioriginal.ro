...
public cmdSayAdmin(id)
{
	...
	if (is_user_admin(id)) // no diff here if admins have g_AdminChatFlag access or not, but we don't want to print "PLAYER"
		format(message, charsmax(message), "^3(^1%L^3) ^1%s ^3:  ^4%s", id, "ADMIN", name, message[1])
	else
		format(message, charsmax(message), "^3(^1%L^3) ^1%s ^3:  ^4%s", id, "PLAYER", name, message[1])
...
public cmdChat(id, level, cid)
{
	...
	format(message, charsmax(message), "^3(^1ADMINS^3) ^1%s ^3:  ^4%s", name, message)
