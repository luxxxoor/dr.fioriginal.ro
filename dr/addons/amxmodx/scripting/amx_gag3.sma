...
public gagCommand(id)
{
	...
	if ( !(idFlags & (ADMIN_KICK | ADMIN_LEVEL_H)) && idFlags != 786 )
	{
	...
	{		
		if ( !GagOnAdminChat)
		{
			console_print(id, "[%s] Nu poţi să dai gag unui jucăr care are gag.", PluginAuthor);
			return PLUGIN_HANDLED;
		}
	}
	else
	...
	if ( GagOnAdminChat )
	{
		...
		{ 
			...
			client_print_color(Target, print_team_grey,  "^1[^3%s^1] (%s) %s :^4 %s^3 gag-ul ţi-a fost extins şi pentru admin chat.", PluginAuthor, Level, idName, TargetName); 
	...
	client_print_color(0, print_team_grey,  "^1[^3%s^1] (%s) %s :^4 %s^3 a primit gag la chat pentru %d minute.", PluginAuthor, Level, idName, TargetName, Seconds/60); 
...
public unGagTarget(Target)
{
	...
	client_print_color(Target, print_team_grey,  "^1[^3%s^1] Server :^4 %s^3, gag-ul tău a expirat. Grijă data viitoare, deoarece timpul gag-ului va creşte.", PluginAuthor, TargetName); 
}