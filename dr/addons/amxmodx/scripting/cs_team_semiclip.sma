...
#define is_user_valid_alive(%1)		(1 <= %1 <= MaxClients && get_bitsum(bs_IsAlive, %1) && !get_bitsum(bs_IsDying, %1))
...
DoRandomSpawn(id, type)
{
	...
	switch (type)
	{
		...
		case 4: /* Trespass */
		{
			...
			iNum = find_sphere_class(id, "player", 102.0, iPlayers, MaxClients)
...
{
	if (!c_iSemiclip || !is_user_connected(ID_RANGE))
		return
	
	static id
	for (id = 1; id <= MaxClients; id++)
	{
		if (!get_bitsum(bs_IsAlive, id) || !is_user_connected(id))
			continue
