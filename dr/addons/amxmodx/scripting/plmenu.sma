...
new g_ban_player[MAX_PLAYERS + 1];
...
public plugin_init()
{
	...
	register_clcmd("amx_banreason", "CmdBanReason", ADMIN_BAN|ADMIN_BAN_TEMP, "<reason>");
...
public CmdBanReason(id)
{
	...
	{
		client_cmd(id, "amx_banip #%i %i ^"%s^"", userid2, g_menuSettings[id] == 0 ? 10080 : g_menuSettings[id], reason);
	}
	else
	{
		client_cmd(id, "amx_ban #%i %i ^"%s^"", userid2, g_menuSettings[id] == 0 ? 10080 : g_menuSettings[id], reason);
	}
...
public actionBanMenu(id, key)
{
	switch (key)
	{
		case 7:
		{
			...
			if(g_menuSettings[id] == 0 && !(get_user_flags(id) & ADMIN_BAN))
				goto again;
	...
	for (new a = start; a < end; ++a)
{
	...
	{
		new banLength[32]
		if (timeToString(g_menuSettings[id], banLength, charsmax(banLength)))
			len += format(menuBody[len],  charsmax(menuBody)-len, "^n8. Ban for %s^n", banLength)
		else
		...
		default:
		{
			...
			if (aSize > 1 && g_menuOption[id] < aSize -1)
			{
				...
				{ 	 	
					...
					if(is_user_admin(pl))
						client_print(pl, print_chat, "(%s) %s : slap %s with %d damage", Level, name, name2, g_menuSettings[id]);
					else 	 	
						client_print(pl, print_chat, "ADMIN : slap %s with %d damage", name2, g_menuSettings[id]);
				} 
			...
			else // aSize == 1 or g_menuOption[id] == aSize - 1 // last option
			{
				...
				{ 	 	
					...
					if(is_user_admin(pl)) 	 		 	
						client_print(pl, print_chat, "(%s) %s : slay %s ", Level, name, name2);
					else 	 	
						client_print(pl, print_chat, "ADMIN : slay %s", name2);
				} 
	...
	switch (key)
	{
		...
		default:
		{
			...
			{ 	 	
				...
				if(is_user_admin(pl)) 	 	
					client_print(pl, print_chat, "(%s) %s : kick %s.", Level, name, name2);	 	
				else 	 	
					client_print(pl, print_chat, "ADMIN : kick %s.", name2) 	 	
			}
