...
public cmdSayChat(id, level)
{
	...
	new name[MAX_NAME_LENGTH], authid[32], userid, Level[35];
	...
	show_hudmessage(0, "(%s) %s :   %s", Level, name, message[i + n])
	client_print(0, print_notify, "(%s) %s :   %s", Level, name, message[i + n])
...
public cmdSayAdmin(id)
{
	...
	new message[192], name[MAX_NAME_LENGTH], authid[32], userid, Level[35];
	...
	format(message, charsmax(message), "(^3%s^1) %s :  %s", Level, name, message[1]) 
	...
	for (new i = 0; i < inum; ++i)
	{
		...
		if (pl == id || get_user_flags(pl) & g_AdminChatFlag)
			client_print_color(pl, print_team_red, "^1%s", message)
	}
...
public cmdChat(id, level, cid)
{
	...
	new name[MAX_NAME_LENGTH], players[MAX_PLAYERS], inum, authid[32], userid, pl, Level[35];
	...
	format(message, charsmax(message), "(^3%s^1) %s :   %s",  Level, name, message)
	...
	for (new i = 0; i < inum; ++i)
	{
		...
		if (access(pl, g_AdminChatFlag))
			client_print(pl, print_chat, "^1%s", message)
	}
...
public cmdSay(id, level, cid)
{
	...
	client_print(0, print_chat, "(ALL) (%s) %s : %s", Level, name, message)
	console_print(id, "(ALL) (%s) %s : %s", Level, name, message)	
...
public cmdPsay(id, level, cid)
{
	...
	new name[MAX_NAME_LENGTH], players[MAX_PLAYERS];
	read_argv(1, name, charsmax(name))
	new plnum = cmd_targetex(id, name, CMDTARGET_NO_MULTIPLE_TARGETS, players);
	...
	client_print(players[0], print_chat, "(%s) %s :   %s", name, name2, message[length])
...
public cmdTsay(id, level, cid)
{
	...
	new found = 0, a = 0
	new lang[3], langnum = get_langsnum(), Level[35];
	...
	show_hudmessage(0, "(%s) %s :   %s", Level, name, message[length])
	client_print(0, print_notify, "(%s) %s :   %s", Level, name, message[length])
	console_print(id, "(%s) %s :   %s", Level, name, message[length])
