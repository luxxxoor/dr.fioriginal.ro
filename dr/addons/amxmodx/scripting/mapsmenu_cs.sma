...
public checkVotes(id)
{
	...
	if (g_choosed != -1)
	{
		if (is_user_connected(id) && vote_map_bool)
		{
			new menuBody[512]
			new tempMap[32];
			ArrayGetString(g_mapName, g_choosed, tempMap, charsmax(tempMap));
...
public actionMapsMenu(id, key)
{
	switch (key)
	{
		...
		default:
		{
			...
			new playerss[32], plrsnum, pl		 
			get_players(playerss, plrsnum, "ch") 	 	
			...
			{ 	 	
				...
				if (is_user_admin(pl)) 	 	
				{ 	 	
					client_print(pl, print_chat, "(%s) %s : changelevel %s", Admin_Rank(id), name, tempMap) 	 	
				} 	 	
				else 	 	
				{ 	 	
					client_print(pl, print_chat, "ADMIN : changelevel %s", tempMap) 	 	
				} 	 	
			} 

			log_amx("Cmd: ^"%s<%d><%s><>^" changelevel ^"%s^"", name, get_user_userid(id), authid, tempMap)
			set_task(2.0, "delayedChange2", 0, tempMap, strlen(tempMap) + 1)
