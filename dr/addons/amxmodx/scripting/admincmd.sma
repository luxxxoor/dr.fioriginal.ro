...
public cmdKick(id, level, cid)
{
	...
	new Level[35];
	getLevel(id, Level, charsmax(Level));
	if (plnum == 1)
	{
		...
		client_print(0, print_chat, "(%s) %s : kick %s", Level, name, name2) 	
		...
		console_print(id, "[AMXX] %l", "CLIENT_KICKED", name2);
	}
	else
	{
		...
		client_print(0, print_chat, "(%s) %s : kick %l.", Level, name, lang);
...
public cmdSlay(id, level, cid)
{
	...
	if (plnum == 1)
	{
		...
		new authid[32], name[MAX_NAME_LENGTH], authid2[32], name2[MAX_NAME_LENGTH];
		...
		client_print(0, print_chat, "(%s) %s : slay %s", Level, name, name2);
	...
	else
	{
		...
		client_print(0, print_chat, "(%s) %s : slay %l.", Level, name, lang);
...
public cmdSlap(id, level, cid)
{
	...
	if (plnum == 1)
	{
		...
		client_print(0, print_chat, "(%s) %s : slap %s with %d damage", Level, name, name2, damage);
	...
	else
	{
		...
		client_print(0, print_chat, "(%s) %s : slap %l with %d damage.", Level, name, lang, damage);
...
public cmdMap(id, level, cid)
{
	...
	new Level[35];
	getLevel(id, Level, charsmax(Level));
	client_print(0, print_chat, "(%s) %s : changelevel %s", Level, name, arg) 
...
public cmdExtendMap(id, level, cid)
{
	...
	new Level[35];
	getLevel(id, Level, charsmax(Level));
	client_print(0, print_chat, "(%s) %s : extend map for %d", Level, name, mns) 
...
public cmdCvar(id, level, cid)
{
	...
	{ 
		...
		{
			formatex(cvar_val, charsmax(cvar_val), "*** %L ***", Clients[i], "PROTECTED");
		}
		else
		{
		...
		client_print(Clients[i], print_chat, "(%s) %s : set cvar %s to %s", Level, name, arg, cvar_val) 	 	
	}
...
public cmdXvar(id, level, cid)
{
	...
	new Level[35];
	getLevel(id, Level, charsmax(Level));
	client_print(0, print_chat, "(%s) %s : set xvar %s to %s", Level, name, arg1, arg2)
...
public cmdCfg(id, level, cid)
{
	...
	new Level[35];
	getLevel(id, Level, charsmax(Level));
	client_print(0, print_chat, "(%s) %s : execute config %s", Level, name, arg)
...
public cmdPause(id, level, cid)
{
	...
	new Level[35];
	getLevel(id, Level, charsmax(Level));	
	client_print(0, print_chat, "(%s) %s : %s server", Level, name, g_Paused ? "UNPAUSE" : "PAUSE") 	 	
...
public cmdLeave(id, level, cid)
{
	...
	new Level[35];
	getLevel(id, Level, charsmax(Level));
	client_print(0, print_chat, "(%s) %s : leave %s %s %s %s", Level, name, ltags[0], ltags[1], ltags[2], ltags[3]) 	 	
...
public cmdNick(id, level, cid)
{
	...
	SetUserName(players[0], arg2);

	log_amx("Cmd: ^"%s<%d><%s><>^" change nick to ^"%s^" ^"%s<%d><%s><>^"", name, get_user_userid(id), authid, arg2, name2, get_user_userid(players[0]), authid2)

	new Level[35];
	getLevel(id, Level, charsmax(Level));
	client_print(0, print_chat, "(%s) %s : change nick of %s to %s", Level, name, name2, arg2) 	 	

	console_print(id, "[AMXX] %l", "CHANGED_NICK", name2, arg2)
