...
public actionVoteMapMenu(id, Menu, Item)
{
	...
	if ((Item+1) % 7 == 0)
	{
		...
		new authid[32], name[MAX_NAME_LENGTH], Level[35];
			
		get_user_authid(id, authid, charsmax(authid))
		get_user_name(id, name, charsmax(name))

		getLevel(id, Level, charsmax(Level));
		client_print(0, print_chat, "(%s) %s : vote map(s)", Level, name) 	 	
...
public actionMapsMenu(id, Menu, Item)
{
	...
	new MapName[32], Name[MAX_NAME_LENGTH], authid[32], Access, Info[1], Callback, Level[35];
	get_user_name(id, Name, charsmax(Name));
	get_user_authid(id, authid, charsmax(authid));
	menu_item_getinfo(Menu, Item, Access, Info, charsmax(Info), MapName, charsmax(MapName), Callback);
	getLevel(id, Level, charsmax(Level));
	client_print(0, print_chat, "(%s) %s: changelevel to %s", Level, Name, MapName);
	log_amx("Cmd: ^"%s<%d><%s><>^" changelevel ^"%s^"", Name, get_user_userid(id), authid, MapName);
	set_task(2.0, "delayedChange2", 0, MapName, strlen(MapName) + 1);
	return PLUGIN_HANDLED;
}
