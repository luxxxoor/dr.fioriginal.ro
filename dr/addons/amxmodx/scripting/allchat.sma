...
new COLCHAR[3][2] = { "^3"/*team col*/, "^4"/*green*/, "^1"/*white*/ }
...
public col_changer( msg_id, msg_dest, rcvr )
{
	...
	if( equal( str2, "#Cstrike_Chat", strlen("#Cstrike_Chat") ) )
	{
		new str3[22]
		...
		if( !strlen( str3 ) )
		{
			...
			new bool:is_team_msg = !bool:equal( str2, "#Cstrike_Chat_All", strlen("#Cstrike_Chat_All") )
			//new bool:is_team_msg = !bool:equal( str2, "#Cstrike_Chat_All", strlen("#Cstrike_Chat_All") )
...
public buildmsg( sndr, is_sndr_spec, is_team_msg, sndr_team, namecol, msgcol, str4[ ] )
{
	...
	if( is_team_msg )
	{
		if( is_sndr_spec ) prefix = "^1(Spectator) "
		else if( sndr_team == 1 ) add( prefix[strlen(prefix)-1], charsmax(prefix), "(Terrorist) " )
		else if( sndr_team == 2 ) add( prefix[strlen(prefix)-1], charsmax(prefix), "(Counter-Terrorist) " )
	}
	
	formatex( msg, 199, "%s%s%s :  %s%s",\
		strlen( prefix ) > 1 ? prefix : "",\
		COLCHAR[namecol], sndr_name, COLCHAR[msgcol], str4 )
	
	/*if (contain(msg, "akm") != -1)
		replace_all(msg, charsmax(msg), "akm", "acum");*/
