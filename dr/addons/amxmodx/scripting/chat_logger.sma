...
public plugin_init()
{
	...
	copy(FilePath, charsmax(FilePath), "/chatlogger");
}
...
public logtext(id)
{
	...
	if(get_pcvar_num(g_cvarlogmode))
	{
		...
		formatex(LogFile, 64, "%s/%s.html", FilePath, datestr)
		...
		formatex(logmsg, MAXLEN, "<font color=yellow>%s &lt;%s&gt;&lt;%s&gt;</font>", timestr, authid, ip)
	}
	else
	{
		formatex(LogFile, 64, "%s/ChatLog.html", FilePath)
		if(!file_exists(LogFile))
		{
	...
	replace_all(said, 191, ">", "&gt;")
	formatex(logmsg, MAXLEN, "%s%s</font> <font color=yellow>:</font> <font color=green>%s</font><br>", logmsg, name, said[pos])
