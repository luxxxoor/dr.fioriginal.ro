...
public plugin_init()
{
	...
	register_clcmd("amx_votebanreason", "CmdBanReason", ADMIN_BAN|ADMIN_BAN_TEMP, "<reason>");
...
public checkVotes()
{
	...
	if (g_execResult)
	{
		...
		if (is_user_connected(g_voteCaller) && vote_map_bool )
		{
...
public cmdVoteKickBan(id, level, cid)
{
	...
	if (voteban)
	{
		...
		add(g_Answer, charsmax(g_Answer), g_ban_reason);
	}
	else
	{
		g_Answer = "kick #%s";
