...
public plugin_init()
{
	...
	//register_concmd("", "addadminfn", ADMIN_RCON, "<playername|auth> <accessflags> [password] [authtype] - add specified player as an admin to users.ini")
...
#else
	format(configsDir, 63, "%s/claw.ini", configsDir)
...
AddAdmin(id, auth[], accessflags[], password[], flags[], comment[]="")
{
	...
	if (sql == Empty_Handle)
	{
		...
		new configsDir[64]
		get_configsdir(configsDir, charsmax(configsDir))
		format(configsDir, charsmax(configsDir), "%s/claw.ini", configsDir)
...
public cmdReload(id, level, cid)
{
	...
	format(filename, charsmax(filename), "%s/claw.ini", filename)
