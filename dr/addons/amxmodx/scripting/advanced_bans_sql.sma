...
new const Server[] = "Dr.FioriGinal.Ro";
...
PrintActivity(const admin_name[], const message_fmt[], any:...)
{
	...
	static message[192], Level[35], AdminName[70];
	getLevel(get_user_index(admin_name), Level, charsmax(Level));
	formatex(AdminName, charsmax(AdminName), "(%s) %s", Level, admin_name);
	vformat(message, charsmax(message), message_fmt, 3);
	
	replace(message, charsmax(message), "$name", AdminName);
