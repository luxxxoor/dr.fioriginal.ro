...
public Wrong_ver()
{
	for(new i=1;i<get_maxplayers();i++)
	{
		if(is_user_alive(i) && is_user_connected(i))
			client_print_color(i, print_team_blue, "^x04Version^x03 uq_jumpstats.amxx^x01(%s)^x04 different from^x03 uq_jumpstats_tops.amxx^x01(%s)",VERSION,plugin_ver);
	}
	set_task(5.0,"Wrong_ver");
}
...
stock kz_make_cvarexec(const config[])
{
	...
	fprintf(f, "^n");
	fprintf(f, "// say !trafes - on/off statistics Strafes^n"); 
...
public Log_script(f_frames,cheated_frames,id,Float:log_dist,Float:log_max,Float:log_pre,log_str,log_sync,jump_type_str[],wpn_str[],punishments[],t_str[40*NSTRAFES])
{
	...
	if(uq_script_notify)
	{
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_SCRIPTNOTIFY",prefix,username,jump_type_str);
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_SCRIPTNOTIFY",prefix,username,jump_type_str);
		client_print_color(id, print_team_grey,"^3%L",LANG_SERVER,"UQSTATS_SCRIPTNOTIFY",prefix,username,jump_type_str);
	}
}
public krasnota(id)
{                
		...
		else
		{
			...
			if( g_alive[id] && team[id]==uq_team)
			{
				...
				if((movetype[id] != MOVETYPE_FLY))	
				{
					...
					if(!(flags&FL_ONGROUND) && SurfFrames[id]<10.1 && uq_fps==1 && fps_hight[id]==false)
					{
						fps_hight[id]=true;
					}
					if((flags&FL_ONGROUND) && SurfFrames[id]>10.1 && fps_hight[id])
					{
						fps_hight[id]=false;
					}
				...
				if(flags&FL_ONGROUND && slidim[id]==true && Pmaxspeed == 250.0)
				{
					...
					{ 
						...
						if(gHasColorChat[ids] ==true || ids==id)
						{
							if(needslide[id]-groundslide[id]==slidedist[id])
							{
								client_print_color(ids, print_team_grey, "[%s] %s jumped %d.xxx units with Slide lj!^x01%s",prefix, g_playername[id], floatround(slidedist[id], floatround_floor),airacel[id]);
							}
							else 
							client_print_color(ids, print_team_grey, "[%s] %s jumped %.3f units with Slide lj!^x01%s",prefix, g_playername[id], slidedist[id],airacel[id]);				
						}
					}
							...
							if( !failed_jump[id] )
							{
										     ...
										     || jump_type[id]==Type_ladderBhop || jump_type[id]==Type_ladder || jump_type[id]==Type_LongJump || jump_type[id]==Type_HighJump))
								{
									if ( distance[id] >= god_dist ) {
										...
										client_print_color(ids, print_team_red, "^3[%s] %s jumped %.3f units with %s!^1%s %s %s ^3%s",prefix, g_playername[id], distance[id],Jtype[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
										...
										client_print_color(ids, print_team_blue, "^3[%s] %s jumped %.3f units with %s!^1%s %s %s ^3%s",prefix, g_playername[id], distance[id],Jtype[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
									else if ( distance[id] >= pro_dist ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/perfect");
									
										client_print_color(ids, print_team_red, "^4[%s] %s jumped %.3f units with %s!^1%s %s %s ^4%s",prefix, g_playername[id], distance[id],Jtype[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
									else if ( distance[id] >=good_dist ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/impressive");
									
										client_print_color(ids, print_team_grey, "^3[%s] %s jumped %.3f units with %s!^1%s %s %s ^3%s",prefix, g_playername[id], distance[id],Jtype[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
								}
								else if(jump_type[id]==Type_Multi_CountJump || (multiscj[id]==2 && jump_type[id]==Type_StandUp_CountJump) || (multidropcj[id]==2 && jump_type[id]==Type_Drop_CountJump))
								{
									if ( distance[id] >= god_dist ) {
										...
										if( uq_light ) krasnota(id);
										client_print_color(ids, print_team_red,"^3[%s] %s jumped %.3f units with %s! (%d ducks)^1%s %s %s ^3%s",prefix, g_playername[id], distance[id],Jtype[id],ducks[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
									else if ( distance[id] >= leet_dist  ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/mod_wickedsick");
										...
										if( uq_light ) sineva(id);
										client_print_color(ids, print_team_blue,"^3[%s] %s jumped %.3f units with %s! (%d ducks)^1%s %s %s ^3%s",prefix, g_playername[id], distance[id],Jtype[id],ducks[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
									else if ( distance[id] >= pro_dist ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/perfect");
									
										client_print_color(ids, print_team_red, "^4[%s] %s jumped %.3f units with %s! (%d ducks)^1%s %s %s ^4%s",prefix, g_playername[id], distance[id],Jtype[id],ducks[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
									else if ( distance[id] >=good_dist ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/impressive");
									
										client_print_color(ids, print_team_grey,"^3[%s] %s jumped %.3f units with %s! (%d ducks)^1%s %s %s ^3%s",prefix, g_playername[id], distance[id],Jtype[id],ducks[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
								}
							}	
						}
...
{	
	if( !gHasColorChat[id] )
	{
		...
		client_print_color(id, print_team_red, "^4[%s]^1 ColorChat enabled. To disable, type !colorchat", prefix);
	}
	else 
	{
		gHasColorChat[id] = false;
		
		client_print_color(id, print_team_red, "^4[%s]^1 ColorChat disabled. To enable, type !colorchat", prefix);
	}
...
public cmdljStats( id ) {
	...
	{
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_LJSTATS_D", prefix);
		g_lj_stats[id]=false;
	...
	else 
	{
		...
		g_lj_stats[id]=true;
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_LJSTATS_E", prefix);
	}
...
{		
	client_print_color(id, print_team_grey,"^3%L",LANG_SERVER,"UQSTATS_VERSION_1", prefix,VERSION);
	
	if(kz_sql==1)
	{
		client_print_color(id, print_team_grey,"^3%L",LANG_SERVER,"UQSTATS_VERSION_SQL1", prefix);
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_VERSION_SQL2", prefix);
	}
	else client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_VERSION_2", prefix);
}
public pre_stats(id)
{	
	if(kz_stats_pre[id]==true) 
	{
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_PRESTATS_D", prefix);
		kz_stats_pre[id]=false;
	}
	else 
	{
		kz_stats_pre[id]=true;
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_PRESTATS_E", prefix);
	}
}
...
{	
	...
	{
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_STRAFESTATS_D", prefix);
		streifstat[id]=false;
	}
	else 
	{
		streifstat[id]=true;
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_STRAFESTATS_E", prefix);
	}
}
public cmdljbeam(id)
{
	if(ingame_strafe[id])
	{
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_BEAMDONTWORK", prefix);
	}
	else
	{
		if(kz_beam[id]==true) 
		{
			client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_BEAMSTATS_D", prefix);
			kz_beam[id]=false;
		}
		else 
		{
			kz_beam[id]=true;
			client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_BEAMSTATS_E", prefix);
		}
	}
}
public show_pre(id)
{
	if(showpre[id]==true) 
	{
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_SHOWPRE_D", prefix);
		showpre[id]=false;
	}
	else 
	{
		showpre[id]=true;
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_SHOWPRE_E", prefix);
	}
}
public show_speed(id)
{
	...
	if(uq_admins==1 && !player_admin[id])
	{
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_SPEED_D1", prefix);
	}
	else if(g_team!=uq_team && !uq_speed_allteam)
	{
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_SPEED_D2", prefix);
	}
	else
	{
		if(jofon[id])
		{
			client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_SPEED_D3", prefix);
		}
		else
		{
			if(speedon[id]==false) 
			{
				client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_SPEED_E", prefix);
				speedon[id]=true;
			...
			else 
			{
				...
				client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_SPEED_D", prefix);
			}
		}
	}
}
public trainer_jof(id)
{
	...
	if(uq_admins==1 && !player_admin[id])
	{
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_JOFTR_D1", prefix);
	}
	else if(g_team!=uq_team)
	{
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_JOFTR_D2", prefix);
	}
	else
	{
		if(speedon[id])
		{
			client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_JOFTR_D3", prefix);
		}
		else
		{
			if(jofon[id]==false) 
			{
				client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_JOFTR_E", prefix);
				jofon[id]=true;
				jof[id]=0.0;
			...
			else 
			{
				...
				client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_JOFTR_D", prefix);
			}
		}
	}
}
public speed_type(id)
{
	...
	{
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_SPEED_B", prefix);
		speedtype[id]=true;
	}
	else 
	{
		speedtype[id]=false;
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_SPEED_S", prefix);
	}
}
public show_jheight(id)
{
	...
	{
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_JHEIGH_E", prefix);
		jheight_show[id]=true;
	}
	else 
	{
		jheight_show[id]=false;
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_JHEIGH_D", prefix);
	}
}
public show_jof(id)
{
	...
	{
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_JOF_E", prefix);
		showjofon[id]=true;
	}
	else 
	{
		showjofon[id]=false;
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_JOF_D", prefix);
	}
}
public show_early(id)
{
	...
	{
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_BHOPWARN_E", prefix);
		failearly[id]=true;
	}
	else 
	{
		failearly[id]=false;
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_BHOPWARN_D", prefix);
	}
}
public multi_bhop(id)
{
	...
	{
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_MULTIBHOP_E", prefix);
		multibhoppre[id]=true;
	}
	else 
	{
		multibhoppre[id]=false;
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_MULTIBHOP_D", prefix);
	}
}
public duck_show(id)
{
	...
	{
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_DUCKSPRE_E", prefix);
		showduck[id]=true;
	}
	else 
	{
		showduck[id]=false;
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_DUCKSPRE_D", prefix);
	}
}
public lj_show(id)
{
	if(ljpre[id]==false) 
	{
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_LJPRE_E", prefix);
		ljpre[id]=true;
	}
	else 
	{
		ljpre[id]=false;
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_LJPRE_D", prefix);
	}
}
public enable_sounds(id)
{
	if(uq_sounds)
	{
		...
		{
			client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_SOUND_E", prefix);
			enable_sound[id]=true;
		}
		else 
		{
			enable_sound[id]=false;
			client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_SOUND_D", prefix);
		}
	}
	else client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_SOUND_D1", prefix);
}
public ShowedgeFail(id)
{
	if(Show_edge_Fail[id]==false) 
	{
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_EDGEF_E", prefix);
		Show_edge_Fail[id]=true;
	}
	else 
	{
		Show_edge_Fail[id]=false;
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_EDGEF_D", prefix);
	}
}
public Showedge(id)
{
	...
	{
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_EDGE_E", prefix);
		Show_edge[id]=true;
	}
	else 
	{
		Show_edge[id]=false;
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_EDGE_D", prefix);
	}
}
public heightshow(id)
{
	...
	{
		client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_FALLH_E", prefix);
		height_show[id]=true;
	}
	else 
	{
		height_show[id]=false;
		client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_FALLH_D", prefix);
	}
}
public ingame_st_stats(id)
{
	if(uq_istrafe)
	{
		if(kz_beam[id])
		{
			client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_ISTRAFE_D1", prefix);
		}
		else
		{
			if(ingame_strafe[id]==false) 
			{
				client_print_color(id, print_team_blue,"^3%L",LANG_SERVER,"UQSTATS_ISTRAFE_E", prefix);
				ingame_strafe[id]=true;
			}
			else 
			{
				ingame_strafe[id]=false;
				client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_ISTRAFE_D", prefix);
			}
		}
	}
	else client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_ISTRAFE_D2", prefix);
...
public ResetHUD(id)
{
	if(is_user_alive(id) && !is_user_bot(id) && !is_user_hltv(id) )
	{
		if(firstshow[id]==false)
		{
			...
			{
				client_print_color(id, print_team_red,"^4[%s]^1 Server runs:^3 100aa.^1 Type !ljtop to show Tops Menu. Type !ljsmenu to show Options Menu", prefix);
			}
			else if( uq_airaccel==0 || uq_airaccel==10)
			{
				client_print_color(id, print_team_red,"^4[%s]^1 Server runs:^3 10aa.^1 Type !ljtop to show Tops Menu. Type !ljsmenu to show Options Menu", prefix);
			}
			else 
				client_print_color(id, print_team_red,"^4[%s]^1 Server runs:^3 %daa.^1 Type !ljtop to show Tops Menu. Type !ljsmenu to show Options Menu", prefix,uq_airaccel);		
				
			firstshow[id]=true;
			
			if(equali(mapname,"slide_gs_longjumps") || equali(mapname,"b2j_slide_longjumps"))
			{
				client_print_color(id, print_team_red,"^3%L",LANG_SERVER,"UQSTATS_SLIDE", prefix);
			}
		}
