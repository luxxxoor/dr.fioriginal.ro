...
public plugin_init() {
    ...
    //register_clcmd("say /scout", "give_stuff", ADMIN_LEVEL_H, "Scout pentru VIP");
    ...
    gcvarDistribute=register_cvar("freezetrap_distribute","1");
    gcvarLimit=register_cvar("freezetrap_limit","1000");
    gcvarCarry=register_cvar("freezetrap_carry","3");
    ...
    register_clcmd("amx_vip","cmd_amx_give_sg",ADMIN_RCON, "<nick>");
    register_clcmd("ancabanca","buyFreezeNade");
...
public give_stuff(id)
    ...
    return 1;
	if (get_user_weapon(id) != CSW_SCOUT || comanda_folosita[id] == false)
	{
...
public resetModel(id, level, cid) {
	if (get_user_flags(id) & ADMIN_LEVEL_H) {
		//new CsTeams:userTeam = cs_get_user_team(id) // aici e problema.
		if (get_user_flags(id) == 1) {
			fm_set_user_model(id, "smith")
		}
		else 
...
if(!is_user_alive(iTarget) || !is_user_connected(id))
return PLUGIN_HANDLED;
...
giveNade(id, bool:bCmd=false){
    ...
    if(giGrenades[id] >= get_pcvar_num(gcvarLimit) && !bCmd){
        client_print(id, print_center, "Ai atins limita.");
        return 0;
    }
